...
import { 
  ...
  Rocket, Target, Map, Terminal, MessageCircle, ChevronDown, Loader2
} from "lucide-react";
...
// 🧠 ADDED THE MISSING FAQS ARRAY BACK 🧠
const faqs = [
  {
...
// Lightweight reusable animation variant so laptops don't blast 🚀
const fadeUpVariant = {
...
export default function LocalizedHome() {
  ...
  const [iframeLoadingStatus, setIframeLoadingStatus] = useState(false);
  ...
  useEffect(() => {
    setMounted(true);
    // Fake preloader for that premium feel
    const timer = setTimeout(() => setLoading(false), 1200);
    
    // Custom smooth cursor tracker
    const updateMousePosition = (e: MouseEvent) => {
      setMousePosition({ x: e.clientX, y: e.clientY });
    };

    // Tracker for hovering over clickable elements to scale the custom cursor
  ...
  const handleProjectSwitch = (project: typeof projectsData[0]) => {
    ...
    setIframeLoadingStatus(false);
  };

  const triggerIframeLoad = () => {
    setIframeLoadingStatus(true);
    // Skeleton buffer before showing iframe
    setTimeout(() => {
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {loading && (
          <motion.div 
            ...
            className="fixed inset-0 z-[99999] bg-white dark:bg-black flex flex-col items-center justify-center text-black dark:text-white"
          >
            <div className="w-12 h-12 bg-blue-600 rounded-lg flex items-center justify-center font-bold text-2xl animate-pulse mb-4 text-white">S</div>
      ...
      {mounted && (
        <motion.div
          className="fixed top-0 left-0 w-8 h-8 border-[1.5px] border-blue-500 rounded-full pointer-events-none z-[9999] hidden md:flex items-center justify-center mix-blend-difference"
          animate={{ 
      ...
      <div className="flex flex-col min-h-screen font-sans max-w-6xl mx-auto px-4 sm:px-6 py-6 md:py-12 gap-16 md:gap-32 relative overflow-hidden text-zinc-900 dark:text-zinc-100 selection:bg-blue-500/30">
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              "image": "https://shani.dev/logo.png",
              "description": "Top-tier Next.js website developer and app developer offering advanced SEO, AEO, GEO, and LLMO services.",
              "url": "https://shani.dev",
              "priceRange": "$$$"
            })
          }}
        ...
        <motion.nav 
          ...
          className="flex justify-between items-center w-full z-20 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm sticky top-4"
        >
        ...
        >
          <motion.div variants={fadeUpVariant} className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-blue-50 dark:bg-blue-900/10 text-blue-600 dark:text-blue-400 text-xs font-semibold border border-blue-100 dark:border-blue-900/30">
          ...
          <motion.h1 variants={fadeUpVariant} className="text-4xl sm:text-6xl md:text-7xl font-bold tracking-tight max-w-4xl leading-tight">
          ...
          <motion.p variants={fadeUpVariant} className="text-base sm:text-lg md:text-xl text-zinc-600 dark:text-zinc-400 max-w-2xl font-medium px-4">
            I build scalable web apps powered by AI, GEO, and traditional SEO. Zero ghosting. Transparent updates. Production-ready code.
          </motion.p>

          <motion.div variants={fadeUpVariant} className="flex flex-col sm:flex-row gap-4 mt-4 w-full sm:w-auto items-center px-4">
        ...
        <motion.section 
          ...
          id="work" 
          className="w-full flex flex-col gap-8 md:gap-10 z-10"
        >
          ...
          <div className="w-full h-[400px] sm:h-[500px] md:h-[650px] mt-8 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 overflow-hidden flex flex-col relative shadow-sm group">
            ...
            {!iframeLoaded ? (
              <div className="flex-1 flex flex-col items-center justify-center p-6 sm:p-8 text-center bg-zinc-50 dark:bg-zinc-950 transition-colors relative">
                {iframeLoadingStatus ? (
                  // Skeleton Loading State
                  <div className="w-full h-full p-8 flex flex-col gap-4 animate-pulse">
                    <div className="w-1/3 h-8 bg-zinc-200 dark:bg-zinc-800 rounded-md mx-auto"></div>
                    <div className="w-full h-32 bg-zinc-200 dark:bg-zinc-800 rounded-md mt-4"></div>
                    <div className="w-2/3 h-32 bg-zinc-200 dark:bg-zinc-800 rounded-md mx-auto"></div>
        ...
        <motion.section 
          ...
          className="w-full z-10 py-10 sm:py-16 hidden sm:block"
        >
        ...
        <motion.section 
          ...
          viewport={{ once: true, margin: "-100px" }}
          variants={{ visible: { transition: { staggerChildren: 0.1 } } }}
          className="w-full z-10 py-10"
        >
          <motion.div variants={fadeUpVariant} className="flex flex-col gap-2 mb-10 md:mb-12 text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-6 sm:gap-8 relative">
            ...
            {processSteps.map((step) => (
              <motion.div variants={fadeUpVariant} key={step.id} className="flex flex-col items-center text-center gap-4">
        ...
        {/* PSYCHOLOGICAL PRICING (MOBILE SWIPEABLE - CROP FIXED) */}
        <motion.section 
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, margin: "-100px" }}
          variants={fadeUpVariant}
          className="w-full z-10 py-12 md:py-16"
        >
          <div className="flex flex-col gap-3 mb-10 md:mb-12 text-center max-w-2xl mx-auto px-4">
            <h2 className="text-3xl md:text-4xl font-bold tracking-tight">Simple, Transparent Pricing</h2>
            <p className="text-zinc-500 text-sm font-medium">Swipe to see plans. High ROI, zero hidden agency fees.</p>
          </div>
          
          {/* Mobile Carousel / Desktop Grid */}
          <div className="flex overflow-x-auto snap-x snap-mandatory hide-scrollbar gap-6 pb-8 md:grid md:grid-cols-3 md:gap-8 max-w-5xl mx-auto pl-4 md:px-4 w-full">
            
            {/* Basic - The Anchor */}
            <div className="min-w-[85vw] md:min-w-0 snap-center p-6 md:p-8 rounded-3xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/30 flex flex-col hover:border-zinc-300 dark:hover:border-zinc-700 transition-colors">
              <h3 className="text-xl font-semibold mb-2">Basic</h3>
              <p className="text-zinc-500 text-sm mb-6 lg:min-h-[48px]">Test the waters. Perfect for validating a core idea quickly. Frontend only.</p>
              <div className="text-4xl md:text-5xl font-bold mb-1">₹999<span className="text-xl text-zinc-400">+</span></div>
              <div className="text-xs text-zinc-400 font-medium tracking-wide uppercase mb-6 opacity-70">Introductory Offer</div>
            ...
            {/* Growth - The Decoy/Target */}
            <div className="min-w-[85vw] md:min-w-0 snap-center p-6 md:p-8 rounded-3xl border-2 border-blue-500 bg-white dark:bg-zinc-900 flex flex-col relative shadow-xl transform md:-translate-y-4 transition-all hover:shadow-2xl">
              ...
              <p className="text-zinc-500 text-sm mb-6 lg:min-h-[48px]">
                The complete package to launch and scale a real business.
              </p>
              <div className="text-4xl md:text-5xl font-bold mb-1 text-black dark:text-white">₹2,999<span className="text-xl text-zinc-400">+</span></div>
              <div className="text-sm text-zinc-400 line-through mb-6">Usually ₹8,500+ at agencies</div>
              
              <ul className="space-y-3 text-sm font-medium mb-8 flex-1">
                <li className="flex items-start gap-2"><CheckCircle2 className="w-4 h-4 text-blue-500 mt-0.5 shrink-0" /> Full web app build</li>
                <li className="flex items-start gap-2"><CheckCircle2 className="w-4 h-4 text-blue-500 mt-0.5 shrink-0" /> Secure Database & Auth</li>
                <li className="flex items-start gap-2"><CheckCircle2 className="w-4 h-4 text-blue-500 mt-0.5 shrink-0" /> Advanced Generative SEO</li>
                <li className="flex items-start gap-2"><CheckCircle2 className="w-4 h-4 text-blue-500 mt-0.5 shrink-0" /> Source Code Ownership</li>
              </ul>
              <a href="#contact" className="w-full py-3 rounded-xl bg-blue-600 text-white text-center font-bold hover:bg-blue-700 transition-colors shadow-md flex items-center justify-center">Launch Your SaaS</a>
            </div>

            {/* Advanced */}
            <div className="min-w-[85vw] md:min-w-0 snap-center p-6 md:p-8 rounded-3xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/30 flex flex-col hover:border-zinc-300 dark:hover:border-zinc-700 transition-colors">
              <h3 className="text-xl font-semibold mb-2">Advanced</h3>
              <p className="text-zinc-500 text-sm mb-6 lg:min-h-[48px]">Built to scale. For complex architectures and bespoke solutions.</p>
              <div className="text-4xl md:text-5xl font-bold mb-1 text-transparent bg-clip-text bg-gradient-to-r from-zinc-800 to-zinc-400 dark:from-white dark:to-zinc-500">Custom</div>
              <div className="text-xs text-zinc-400 font-medium tracking-wide uppercase mb-6 opacity-70">Limited Availability</div>
              
              <ul className="space-y-3 text-sm font-medium mb-8 flex-1">
                <li className="flex items-start gap-2"><CheckCircle2 className="w-4 h-4 text-zinc-400 mt-0.5 shrink-0" /> Advanced LLMO Integrations</li>
                ...
                <li className="flex items-start gap-2"><CheckCircle2 className="w-4 h-4 text-zinc-400 mt-0.5 shrink-0" /> Dedicated Retainer</li>
              </ul>
            ...
            {/* SPACER TO FIX MOBILE SCROLL CROP */}
            <div className="w-4 shrink-0 md:hidden" aria-hidden="true"></div>

          </div>
        </motion.section>

        {/* TESTIMONIALS (MOBILE SWIPEABLE - CROP FIXED) */}
        <motion.section 
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, margin: "-100px" }}
          variants={{ visible: { transition: { staggerChildren: 0.1 } } }}
          className="w-full z-10 py-8"
        >
          <motion.div variants={fadeUpVariant} className="text-center mb-10 px-4">
          ...
          <div className="flex overflow-x-auto snap-x snap-mandatory hide-scrollbar gap-4 pb-8 md:grid md:grid-cols-2 lg:grid-cols-4 md:gap-6 pl-4 md:px-4">
            {testimonials.map((t, i) => (
              <motion.div 
                key={i}
                variants={fadeUpVariant}
                className="min-w-[80vw] md:min-w-0 snap-center p-6 sm:p-5 rounded-2xl bg-gradient-to-br from-zinc-50 to-zinc-100 dark:from-zinc-900/40 dark:to-zinc-800/40 border border-zinc-200 dark:border-zinc-700 flex flex-col justify-between shadow-lg hover:shadow-xl transition-all duration-300 md:hover:scale-105 hover:border-blue-300 dark:hover:border-blue-600 group"
              >
                <div className="mb-4">
        ...
        <motion.section 
          ...
          viewport={{ once: true, margin: "-100px" }}
          variants={{ visible: { transition: { staggerChildren: 0.1 } } }}
          className="w-full z-10 py-12 md:py-16 max-w-3xl mx-auto px-4"
        >
          ...
          <div className="flex flex-col gap-3">
            {faqs.map((faq, index) => (
              <motion.div variants={fadeUpVariant} key={index} className="border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden bg-white/50 dark:bg-zinc-900/30">
                <button 
